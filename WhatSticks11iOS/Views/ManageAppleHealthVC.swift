...
class ManageAppleHealthVC: TemplateVC {
    ...
    var arryStepsDict = [AppleHealthQuantityCategory](){
        didSet{
            SentrySDK.capture(message: "user_name: \(String(describing: userStore.user.username)) Variable values - arryStepsDict.count: \(arryStepsDict.count)")
            actionGetSleepData()
        }
    }
    var arrySleepDict = [AppleHealthQuantityCategory](){
        didSet{
            SentrySDK.capture(message: "user_name: \(String(describing: userStore.user.username)) Variable values - arrySleepDict.count: \(arrySleepDict.count)")
            actionGetHeartRateData()
        }
    }
    var arryHeartRateDict = [AppleHealthQuantityCategory](){
        didSet{
            SentrySDK.capture(message: "user_name: \(String(describing: userStore.user.username)) Variable values - arryHeartRateDict.count: \(arryHeartRateDict.count)")
...
//                self.spinnerScreenLblMessage(message: "Retrieved \(formatted_arryWorkoutDictCount) Workout records")
    ...
    func sendAppleWorkouts(){
        ...
        guard let user_id = userStore.user.id else {
            ...
            return}
        let all_data_count = arrySleepDict.count + arryStepsDict.count + arryHeartRateDict.count + arryExerciseTimeDict.count + arryWorkoutDict.count
        if all_data_count > 0 {
            self.removeLblMessage()
            
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                let formatted_all_data_count = formatWithCommas(number: all_data_count)
                self.spinnerScreenLblMessage(message: "Sending Apple Health \(formatted_all_data_count) records to \nWhat Sticks API")
            }
        }
    ...
    func sendAppleHealthData(userMessage:String, dateStringTimeStamp:String){
        ...
        guard let user_id = userStore.user.id else {
            self.templateAlert(alertMessage: "No user id. check ManageAppleHealthVC sendAppleHealthData.")
            return}
        let all_data_count = arrySleepDict.count + arryStepsDict.count + arryHeartRateDict.count + arryExerciseTimeDict.count
        if all_data_count > 0{
            let arryAppleHealthData = arrySleepDict + arryStepsDict + arryHeartRateDict + arryExerciseTimeDict
