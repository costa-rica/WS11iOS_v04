...
//        self.locationFetcher.fetchLocation()
    ...
    func requestLogin(){
        userStore.callLoginUser(email: txtEmail.text ?? "", password: txtPassword.text ?? "") { responseResultLogin in
            ...
            case let .success(user_obj):
                ...
                // Sentry
                if let unwp_user_id = user_obj.id{
                    let user = Sentry.User(userId: unwp_user_id)
    ...
    func goToDashboard(){
//        objCreateCrash.topAnchor.constraint(equalTo: view.topAnchor).isActive=true
